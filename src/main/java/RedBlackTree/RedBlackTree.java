...
*            6 12   18  37   48  66 88
* - 2-3树具有以下性质：
*   1. 满足 BST 的基本性质 —— 小于节点值的元素放在左边，大于节点值的元素放在右边
*   2. 每个节点可能是"2节点"或"3节点"（有2个或3个子节点），可以存放1个或2个元素
*   3. 可以保持绝对平衡 —— 从根节点到任意叶子节点所经过的节点数相同（这是我们讲的唯一一种绝对平衡树）
* - 2-3树的 add 操作的精妙机制是其保持绝对平衡的根本原因，SEE：https://coding.imooc.com/lesson/207.html#mid=15081（非常精彩！）
*
* - 但是要让一个节点存放1-2个值、具有2-3个子节点，在编程上会很复杂，而红黑树就由此而来 —— 用来化简2-3树的复杂度。
* - 具体做法是将2-3树中的3节点用一红、一黑两个节点表示，例如上图中的 17 33 这个3节点在红黑树中就会表示成，红色的17和黑色的33。
*   因此整棵树就变成了这样：
*                         43
*                     /       \
*               17 - 33        50         其中 17 是红节点
