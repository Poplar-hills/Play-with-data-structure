 ...
 *     - 而对于链表来说正相反：
 *       - 对头部的操作（addFirst/removeFirst）是 O(1)
 *       - 对尾部的操作（addLast/removeLast）是 O(n)
...
public class LinkedList<E> {
    ...
    * */
    ...
    private Node removeElement(Node node, E e) {  // 不同于 remove，该方法不再需要获取 prev 节点，只需要判断当前节点是否是待删除节点，如果是的话直接断开与下个节点的链接，并返回下个节点即可。
        if (node == null)
            return null;
