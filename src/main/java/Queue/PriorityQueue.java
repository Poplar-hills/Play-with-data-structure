...
*
* - 实现：与普通队列的区别在于 enqueue 和 dequeue 这2个方法。
*
...
*     2. 非线性数据结构：最大堆
*     注：最大堆也可以使用线性数据结构来实现，但它属于非线性数据结构 SEE: MaxHeap。
*   - 不同的实现时间复杂度不同：
*                    enqueue    dequeue
*      普通线性结构      O(1)      O(n)      （如果用类似动态数组的线性结构来实现，则每次出队的时候都需要遍历找到最大元素）
*      顺序线性结构      O(n)      O(1)      （如果还是线性数据结构，但是在入队的时候进行排序）
