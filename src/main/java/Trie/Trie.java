...
*   HashMap 差一些。另一个重要的时间开销在于，由于 Trie 消耗的空间比较大，所以给这些承载指针的空间（TreeMap，HashMap
*   或者数组）开辟内存也是一个额外的时间消耗。
...
*                / | \
*
* - 每个节点有三个子节点，如果待查找字符 == 当前节点中的字符，则选择中间路径；如果 < 节点中的字符，则选择左边路
*   径；如果 > 节点中的字符，则选择右边路径。中间分支存储的是在和该节点匹配成功的情况下的下一个字母，这点和 Trie
*   是一样的。而匹配不成功，则忽略当前节点，去左边或者右边继续找这个字母。这点和 Trie 不同，在 Trie 中只可能匹配
*   成功，每下降一层，一定找的是一个新的字母。
* - 三分搜索树的时间复杂度也是和要搜索的字符串的长度成正比，而空间复杂度也得到了控制。
* */
