...
*   1. 数学中集合类的实现
*   2. 可以高效地回答网络中两个节点直接是否连接的问题（但是不回答两个节点间的路径问题）
* - 而网络可以用于非常多的场景：
*   1. 社交网络的团关系分析
...
*
* - 并查集的两个操作：
*   1. union(p, q) -- 将编号为 p, q 的元素以及他们所在的集合在并查集内连接起来
*   2. isConnected(p, q) -- 判断编号为 p, q 的元素是否属于同一个集合
*
* - 在 UnionFind1 这个实现中，使用数组构建并查集：
*   元素编号： 0  1  2  3  4    union(2,4)   0  1  2  3  4    union(4,0)   0  1  2  3  4    isConnected(2,0)
*            -------------       --->      -------------      --->       -------------          --->         true
