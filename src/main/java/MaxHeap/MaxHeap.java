...
*      19   17 15
*   - 这样的堆叫"最大堆"（max heap）；
*   - 若堆中任意一个节点的值都 <= 它的子节点的值，叫"最小堆"（min heap）。
*
* - 实现：因为二叉堆是一个完全二叉树，而表达完全二叉树有一个经典的实现方式：因为树中的元素是从上到下、从大到小一层
*   一层排列的，因此可以用数组的形式来表示这棵树：[62, 41, 30, 28, 16, 22, 13, 19, 17, 15]。
*   注：1). 二叉树同样也可以用数组表示，但因为不一定是满二叉树，所以数组中可能会出现 null，
*          SEE: play-with-alogrithm-interview - L297_SerializeAndDeserializeBinaryTree）。 
*      2). 用数组表示也可以分广度优先和深度优先，这种将树中元素从上到下一层一层放入数组的是广度优先。
* - 用数组表示二叉堆时，唯一需要解决的问题就是如何找到某一个节点的左右孩子。而用数组表示后，我们可以找到规律：
*   设树上某一个节点在数组中的索引是 i，则有：
*     - 其左孩子的索引: 2 * i + 1
*     - 其右孩子的索引: 2 * i + 2 (右孩子总是在左孩子右边)
*     - 其父节点的索引: (i - 1) / 2 (整型除法会抹去小数)（能找到父节点索引，这是用数组表示的额外好处）
*
* - 操作：对于堆来说：
*   - 唯一的取值操作就是取出堆顶元素（对于最大堆来说就是取出最大值，最小堆就是取出最小值）；
...
public class MaxHeap<E extends Comparable<E>> {
    ...
    private void siftUp(int k) {
        while (k > 0 && data.get(getParentIndex(k)).compareTo(data.get(k)) < 0) {  // 若父节点值大于子节点值则停止 sift up
