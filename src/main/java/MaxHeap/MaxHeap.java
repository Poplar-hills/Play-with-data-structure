...
/*
* - 树这种数据结构有很多变体：BST、AVL tree、Red-black tree、Heap、Trie、Segment tree 等等。而堆（heap）就是其中一种。
* - 在了解堆之前需要先了解"优先队列"（Priority Queue）：一种出队顺序和入队顺序无关，仅和优先级相关的结构。SEE: Queue - PriorityQueue
...
*
* - 实现：因为二叉堆是一个完全二叉树，而表达完全二叉树有一个巧妙的手段：因为树中的元素是从上到下、从大到小一层一层排列的，
*   因此可以用数组的形式来表示这棵树：[62, 41, 30, 28, 16, 22, 13, 19, 17, 15]。
* - 用数组表示二叉堆时，唯一需要解决的问题就是如何找到某一个节点的左右孩子。而用数组表示后，我们可以找到规律：
*   设某一个节点在数组中的 index 是 i，则有：
*     - 其左孩子的 index: 2 * i + 1
*     - 其右孩子的 index: 2 * i + 2 (右孩子总是在左孩子右边)
*     - 其父节点的 index: (i - 1) / 2 (整型除法会抹去小数)（还能找到父节点，这是用数组表示的额外好处）
*
* - 对于堆来说，唯一的取值操作就是取出堆顶元素。
*
